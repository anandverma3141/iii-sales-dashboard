  ...
  return (
    <div className="relative w-[128px]" ref={dropdownRef}>
      ...
      <button
        ...
        className="w-full text-[#2B303466] text-[12px] px-2 py-1 bg-[#FCFDFD] border border-[#D5D5D5] rounded-sm flex justify-between items-center"
      >
        {selected}
        <svg
        ...
        <div className="absolute left-0 w-full w-[128px] bg-[#FCFDFD]  shadow-lg rounded-sm mt-1 border border-[#D5D5D5] z-10">
          <ul className="py-1">
            {options.map((option, index) => (
              <li
                key={index}
                className="px-2 py-1 hover:bg-gray-100 cursor-pointer text-[12px]  text-[#2B303466]"
                onClick={() => {
